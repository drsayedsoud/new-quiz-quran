...
// 1. Update Solo Card Long Press
// Find the solo card div
const soloCardRegex = /<div class="card" onclick="openSoloMixModal\(\)" onmouseenter="playHover\(\)" style="position: relative; border-radius: 20px; padding: 30px 20px; display: flex; flex-direction: column; justify-content: center; align-items: center; background: rgba\(255, 255, 255, 0.05\); backdrop-filter: blur\(10px\); border: 1px solid rgba\(255,255,255,0.1\); overflow: hidden; box-shadow: 0 8px 32px rgba\(0, 0, 0, 0.2\); transition: all 0.3s cubic-bezier\(0.25, 0.8, 0.25, 1\);">/;
const newSoloCard = `<div class="card" onmousedown="startSoloPress()" onmouseup="cancelSoloPress()" onmouseleave="cancelSoloPress()" ontouchstart="startSoloPress()" ontouchend="cancelSoloPress()" onmouseenter="playHover()" style="position: relative; border-radius: 20px; padding: 30px 20px; display: flex; flex-direction: column; justify-content: center; align-items: center; background: rgba(255, 255, 255, 0.05); backdrop-filter: blur(10px); border: 1px solid rgba(255,255,255,0.1); overflow: hidden; box-shadow: 0 8px 32px rgba(0, 0, 0, 0.2); transition: all 0.3s cubic-bezier(0.25, 0.8, 0.25, 1); user-select: none; -webkit-user-select: none;">`;
html = html.replace(soloCardRegex, newSoloCard);

// 2. Add the JS for Long Press
const longPressJs = `
<script>
    let soloCardTimer;
    ...
    function startSingleQuiz(category, mode, juz = null) {
        if(mode === 'solo') {
            ...
            // It's a multiplayer room
            let qCount = 10;
            let roomCategory = category;
            if (juz && juz !== 'all') {
                roomCategory = category + "_juz_" + juz;
            }
            createMultiplayerRoomWithParams(roomCategory, qCount);
        }
    }
...
function getCard(icon, title, color, onclickAttr, desc = "") {
    ...
    <div class="category-card" onclick="${onclickAttr}" style="background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); padding: 15px 20px; border-radius: 15px; display: flex; align-items: center; gap: 20px; cursor: pointer; transition: 0.3s;" onmouseenter="this.style.background='rgba(255,255,255,0.1)'; this.style.transform='scale(1.02)';" onmouseleave="this.style.background='rgba(255,255,255,0.05)'; this.style.transform='scale(1)';">
        ...
        <div style="text-align: right; flex-grow: 1;">
            ...
            ${desc ? \`<div style="font-size: 0.85em; color: #a0aec0; margin-top: 5px;">\${desc}</div>\` : ''}
        </div>
    </div>`;
}

const juzOptionsHtml = (mode) => `
    <div id="${mode}-quran-options" style="display: none; flex-direction: column; gap: 15px; margin-top: 10px;">
